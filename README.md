...
> A design system for h-design UI components, built on [@design-systems/cli](https://github.com/intuit/design-systems-cli).

## 👍 Philosophy

This is a design system intended for UI components currently being built or tested in h-design.
...
For an example we will try to use the `@h-design/card` component.

```sh
npm i @h-design/card
# or with yarn
yarn add @h-design/card
```
...
Now whenever you import one of the `@h-design` components, `babel` will automatically include the correct CSS file!
